...
struct S32K3AdcState {
    ...
    uint32_t tca1;
    uint32_t tca2;
    uint32_t regs[0x100];   /* 影子：未实现偏移读回写值 */
    uint32_t wtimr;
    uint32_t thrhlr[4];
...
{
    if (size == 8) {
        uint64_t lo = s32k3_adc_read(opaque, addr, 4);
        uint64_t hi = s32k3_adc_read(opaque, addr + 4, 4);
        return lo | (hi << 32);
    }
    if (size == 2) {
        uint32_t full = s32k3_adc_read(opaque, addr & ~3u, 4);
...
{
    if (size == 8) {
        s32k3_adc_write(opaque, addr, value & 0xFFFFFFFF, 4);
        s32k3_adc_write(opaque, addr + 4, value >> 32, 4);
        return;
    }
    if (size == 2) {
        uint32_t full = s32k3_adc_read(opaque, addr & ~3u, 4);
        ...
        uint32_t merged = (addr & 2) ? ((full & 0xFFFF) | (w << 16))
                                     : ((full & 0xFFFF0000u) | w);
        s32k3_adc_write(opaque, addr & ~3u, merged, 4);
        return;
    }
    if (size == 1) {
        uint32_t full = s32k3_adc_read(opaque, addr & ~3u, 4);
