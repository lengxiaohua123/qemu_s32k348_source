...
static void s32k3_lpi2c_reset(DeviceState *dev)
{
    ...
    s->mcr = 0;
    s->msr = MSR_TDF;   /* 复位：仅 TDF=1（RM 复位 0000_0001h） */
    s->mier = 0;
    s->mcfgr0 = 0;
    s->mcfgr1 = 0;
    s->mcfgr2 = 0;
...
{
    if (size == 8) {
        uint64_t lo = s32k3_lpi2c_read(opaque, addr, 4);
        uint64_t hi = s32k3_lpi2c_read(opaque, addr + 4, 4);
        return lo | (hi << 32);
    }
    if (size == 2) {
        uint32_t full = s32k3_lpi2c_read(opaque, addr & ~3u, 4);
...
{
    if (size == 8) {
        s32k3_lpi2c_write(opaque, addr, value & 0xFFFFFFFF, 4);
        s32k3_lpi2c_write(opaque, addr + 4, value >> 32, 4);
        return;
    }
    if (size == 2) {
        uint32_t full = s32k3_lpi2c_read(opaque, addr & ~3u, 4);
        ...
        uint32_t merged = (addr & 2) ? ((full & 0xFFFF) | (w << 16))
                                     : ((full & 0xFFFF0000u) | w);
        s32k3_lpi2c_write(opaque, addr & ~3u, merged, 4);
        return;
    }
    S32K3Lpi2cState *s = opaque;
    uint32_t v = value;
